...
void    ImGui::EndTable()
{
    ...
    // (FIXME: To allow auto-fit and allow desirable effect of SameLine() we dissoce 'used' vs 'ideal' size by overriding
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ...
    // We cannot draw neither the cell or row background immedely as we don't know the row height at this point in time.
    switch (target)
    {
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    // Differente column ID with an arbitrary prefix for cases where users name their columns set the same as another widget.
