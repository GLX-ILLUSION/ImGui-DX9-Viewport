...
struct ImGuiDataTypeInfo;           // Type information assoced to a ImGuiDataType enum
struct ImGuiDockContext;            // Docking system context
struct ImGuiDockRequest;            // Docking system dock/undock queued request
struct ImGuiDockNode;               // Docking system node (hold a list of Windows OR two child dock nodes)
struct ImGuiDockNodeSettings;       // Storage for a dock node in .ini file (we preserve those even if the assoced dock node isn't active during the session)
...
// Honor constructor/destructor. Add/remove invalidate all pointers. Indexes have the same lifetime as the assoced object.
...
// Type information assoced to one ImGuiDataType. Retrieve with DataTypeGetInfo().
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    float                   CursorAnim;             // timer for cursor blink, reset on every user action so the cursor reappears immedely
...
struct ImGuiPopupData
{
    ...
    ImGuiID             OpenParentId;   // Set on OpenPopup(), we need this to differente multiple menu sets from each others (e.g. inside menu bar vs loose menu items)
...
struct ImGuiContext
{
    ...
    bool                    ActiveIdHasBeenEditedBefore;        // Was the value assoced to the widget Edited over the course of the Active state.
    ...
    float                   ColorEditLastHue;                   // Backup of last Hue assoced to LastColor[3], so we can restore Hue in lossy RGB<>HSV round trips
    float                   ColorEditLastSat;                   // Backup of last Saturation assoced to LastColor[3], so we can restore Saturation in lossy RGB<>HSV round trips
...
struct IMGUI_API ImGuiWindow
{
    ...
    short                   BeginOrderWithinParent;             // Begin() order within immede parent window, if we are a child window. Otherwise 0.
...
namespace ImGui
{
    ...
    IMGUI_API void          MarkItemEdited(ImGuiID id);     // Mark data assoced to given item as "edited", used by IsItemDeactivatedAfterEdit() function.
    ...
    // - If you intend to split the node immedely after creation using DockBuilderSplitNode(), make sure
    ...
    // Template functions are instanted in imgui_widgets.cpp for a finite number of types.
