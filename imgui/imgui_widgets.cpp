...
// - PressedOnDragDropHold can generally be assoced with any flag.
// - PressedOnDoubleClick can be assoced by PressedOnClickRelease/PressedOnRelease, in which case the second release event won't be reported.
//------------------------------------------------------------------------------------------------------------------------------------------------
...
bool ImGui::DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    {
        format = "%lf"; // scanf differente float/double unlike printf which forces everything to double because of ellipsis
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() + 50.0f; // Adicionando 20 ao comprimento da Slider
    ...
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10, grab_bb.GetCenter().y), ImColor(0, 0, 255, 255)); //slider Line color
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 4, ImColor(0, 0, 255, 255)); //Slider thumb color
    }
...
// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(ImVec2(frame_bb.Min.x + 150.0f, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y + 25), value_buf, value_buf_end, NULL, ImVec2(0.5, 0.5));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (recycle_state)
        {
            ...
            // Note a single mouse click will override the cursor/position immedely by calling stb_textedit_click handler.
            state->CursorClamp();
        }
        else
        {
    ...
    // We have an edge case if ActiveId was set through another widget (e.g. widget being swapped), clear id immedely (don't wait until the end of the function)
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            // Apply new value immedely - copy modified buffer back
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                draw_pos.y += (draw_window->Scroll.y - scroll_y);   // Manipulate cursor pos immedely avoid a frame of lag
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            ...
            {
                // FIXME: Hackily differenting using the DragInt (ActiveId != 0 && !ActiveIdAllowOverlap) vs. using the InputText or DropTarget.
    ...
    if (value_changed)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            B = col[2];
            ColorConvertRGBtoHSV(R, G, B, H, S, V);
            if (memcmp(g.ColorEditLastColor, col, sizeof(float) * 3) == 0) // Fix local Hue as display below will use it immedely.
            {
                if (S == 0)
                    H = g.ColorEditLastHue;
                if (V == 0)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // Refresh tab width immedely, otherwise changes of style e.g. style.FramePadding.x would noticeably lag in the tab bar.
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window)
{
    ...
    if (!window->HasCloseButton)
        tab_flags |= ImGuiTabItemFlags_NoCloseButton;       // Set _NoCloseButton immedely because it will be used for first-frame width calculation.
